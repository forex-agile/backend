...
public class FxRateServiceTest {
    ...
    void testFindFxRateByCurrency_WhenFxRateExists() {
        when(fxRateRepo.findByCurrency(currency)).thenReturn(Optional.of(fxRate));
        FxRate foundFxRate = fxRateService.findFxRateByCurrency(currency);
    ...
    void testFindFxRateByCurrency_WhenFxRateDoesNotExist() {
        ...
        when(fxRateRepo.findByCurrency(invalidCurrency)).thenReturn(Optional.empty());
        assertThrows(RecordNotFoundException.class, () -> {
            fxRateService.findFxRateByCurrency(invalidCurrency);
